...
function test() {
  return log =>
    iter.forEach(loadPackages(), {log, build: 'test'})((lernaPackage, log) => {
...
function prepush(log) {
  const syncNvmRc = () => {
    ...
    return iter.parallel(loadPackages(), {log})(pkg =>
      exec
        .command(pkg)(`cp ${process.cwd()}/.nvmrc .`)
        .catch(e => e)
    )
  }
  ...
  const syncNpmVersion = () => {
    ...
    return iter.parallel(loadPackages(), {log})(pkg =>
      exec
        .command(pkg)(`cp ${process.cwd()}/.npmversion .`)
        .catch(e => e)
    )
  }
...
function pullreq(log) {
  const buildAll = () =>
    iter.forEach(loadPackages(), {log})((lernaPackage, log) =>
      exec.script(lernaPackage, {log})('build')
    )
  const testChanged = () =>
    iter.forEach(filters.gitSince(loadPackages(), {log})('origin/master'), {log})(
      (lernaPackage, log) => exec.script(lernaPackage, {log, silent: false})('test')
    )
